...
class LLMInlineCompletionProvider {
    async provideInlineCompletionItems(document, position, context, token) {
        ...
        const completionItem = new vscode.InlineCompletionItem(pendingCompletion.text, new vscode.Range(position, position));
...
function activate(context) {
    ...
    const inlineCompletionProvider = vscode.languages.registerInlineCompletionItemProvider({ pattern: '**' }, new LLMInlineCompletionProvider());
...
async function generateLLMCompletion(selectedText) {
    // Get configuration settings
    const config = vscode.workspace.getConfiguration('llcg');
    const model = config.get('modelName') || 'Question_reformer_qwen:latest';
    const ollamaUrl = config.get('ollamaUrl') || 'http://localhost:11434/api/generate';
    // Chat templates
